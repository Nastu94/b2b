<div class="p-6 space-y-4">
    ...
    <div class="bg-white shadow rounded-lg p-4 space-y-2">
        ...
        <div><strong>Ordine PS:</strong> {{ $booking->prestashop_order_id }} / {{ $booking->prestashop_order_line_id }}</div>
        <div><strong>Totale:</strong> {{ $booking->total_amount }}</div>

        <div class="pt-2">
            <strong>Cliente:</strong>
            <div class="text-sm text-gray-700">
                {{ data_get($booking->customer_data, 'name') }}
                — {{ data_get($booking->customer_data, 'email') }}
                — {{ data_get($booking->customer_data, 'phone') }}
            </div>
        </div>
    </div>
